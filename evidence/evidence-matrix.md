...
Status: claim-control document with two preserved measured development benchmarks, frozen Windows preflights, and a reviewed public `v0.1.0` Windows x64 preview; formal holdout not run
Created: 2026-08-31  
Project author and evidence owner: LAI ZEYU (来泽宇)

## Purpose

This matrix prevents future application, résumé, portfolio, demo, and release statements from outrunning the available evidence. Individual rows link to implementation, test, and package records; the matrix by itself does not extend those records into clean-target installation, trusted platform signing, Microsoft Store certification or availability, adoption, or use by a real organisation.
...
| Built a working end-to-end analysis product | TESTED — historical exact-DMG E2E plus tagged Windows x64 workflow | Historical Mac records plus `evidence/releases/0.1.0/windows/v0.1.0/packaged-e2e-installed-report.json` and lifecycle report | Historical exact-DMG workflow; tagged Windows Actions run `33409332005` installed the NSIS package and exercised mapping, bundled local analysis, results, export, and uninstall | App 0.1.0; historical Mac artifact has no build-time source commit; Windows tagged commit `b2a226ac9e2623ba210a192095d2feb2eb7dacf4` | “Built and tested an offline workflow on the recorded build Mac and in an installed application on the recorded Windows-hosted runner” | “Production-ready”, “enterprise-grade”, or clean consumer-target Windows/macOS compatibility |
...
| Generated evidence-linked reports | TESTED — synthetic packaged workflows on the recorded Mac and Windows runner | Historical Mac E2E records plus `evidence/releases/0.1.0/windows/v0.1.0/packaged-e2e-installed-report.json` | The packaged workflows exported JSON + HTML and asserted schema, author, dataset hash, and absence of recognized raw-row keys | Report schema `causalpilot.evidence.v1`, app 0.1.0, platform/artifact identity, synthetic data hash | “Exported an aggregate JSON/HTML evidence bundle in the recorded synthetic packaged workflows” | “Hallucination-free”, every possible raw-data shape is detected, or externally audited reporting |
| Protects raw local data from optional external AI | TESTED — limited architecture/export boundary on recorded package tests | Main/preload/sidecar source, export sanitizer, historical package E2E, and `evidence/releases/0.1.0/windows/v0.1.0/packaged-e2e-installed-report.json` | Inspect the relevant exact build and run package E2E; a preserved independent network capture remains future work | App 0.1.0, offline local-engine mode, platform/artifact identity, test date | “In the tested workflows, the local sidecar processed the CSV and the exported bundle contained no recognized raw-row collection” | “Zero data risk”, “no byte can ever leave the device”, or vendor-retention claims without network proof |
| Provides a Marketing Analytics case study | TESTED — synthetic demonstration only | Frozen sample metadata, historical exact-DMG result, and tagged Windows installed packaged E2E result | Re-run the packaged synthetic checkout-incentive workflow and verify dataset SHA-256 `47ab1a639eecfbb1630063298d6d3b447bac5123e87d6e9653bdc283bf9de244` | Synthetic case version/hash, app 0.1.0, platform/artifact identity; Windows tagged commit `b2a226ac…`, historical Mac build does not | “Demonstrated a synthetic marketing A/B workflow with the recorded question, method, result, and limitations” | Invented client, revenue, adoption, or real-world uplift |
...
| Packaged a desktop or installable release | PUBLICLY AVAILABLE — historical arm64 DMG remains local; tagged Windows x64 preview is public | Historical Mac package record/checksums plus `evidence/releases/0.1.0/windows/v0.1.0/github-release-record.md` and `SHA256SUMS-windows.txt` | Historical DMG hash/verify/mounted E2E; tagged Windows run `33409332005` built and hashed NSIS + ZIP from commit `b2a226ac…`; public downloads were rehashed | App 0.1.0; Mac arm64 SHA-256 `c5e6d656…`; public Windows installer `3d8f12ea…` and ZIP `76fca35d…` | “Published a reviewed Windows x64 preview on GitHub and packaged a historical Apple-silicon build” | “Installed on a clean target”, trusted-signed, Store-available, adopted, or production-proven |
| Installed and launched on a clean target | TESTED — hosted-runner installer lifecycle; not clean-target INSTALLED | Historical Mac launch record plus tagged Windows lifecycle and installed packaged E2E reports | The DMG was launched from a read-only mount on the build Mac; the tagged NSIS package was silently installed, exercised, silently uninstalled, and its main executable confirmed removed on the GitHub-hosted runner; separate clean-device installation remains required | Artifact/commit identity, platform/architecture, install/uninstall exit codes, test date | “Completed an automated install, installed-app workflow, and uninstall smoke on the recorded Windows Server 2022 runner” | “Clean-installed”, general Windows 10/11 compatibility, complete residue removal, “works on all Macs”, or Intel compatibility |
...
| Made the source code publicly available | PUBLICLY AVAILABLE — source only | Public repository `https://github.com/lzy2767865503-pixel/causalpilot-ai`, annotated tag, and unauthenticated HTTP checks | The repository and `v0.1.0` release returned HTTP 200; the tag resolves to `b2a226ac9e2623ba210a192095d2feb2eb7dacf4` | Repository URL, tag/commit, check date | “Published the CausalPilot AI source repository on GitHub” | Implying that source publication alone proves any installer, Store listing, adoption, or production use |
| Made a binary release publicly available | PUBLICLY AVAILABLE — GitHub pre-release only | Public `v0.1.0` release, tagged workflow 33409332005, final checksums, and `evidence/releases/0.1.0/windows/v0.1.0/github-release-record.md` | Tag build passed; draft assets were downloaded and rehashed; release and asset endpoints returned unauthenticated HTTP 200; public checksum copy matched | Tag `v0.1.0`, commit `b2a226ac…`, publication `2026-08-31T15:46:21Z`, installer `3d8f12ea…`, ZIP `76fca35d…` | “Published a reviewed Windows x64 preview at the exact GitHub Release URL” | “Available in Microsoft Store”, trusted-signed, broadly Windows-compatible, adopted, or production-proven |
...
The source repository is public. At 2026-08-31T15:46:21Z, tagged commit
`b2a226ac9e2623ba210a192095d2feb2eb7dacf4` was published as the reviewed
`v0.1.0` Windows x64 GitHub pre-release. This makes only the named installer,
ZIP, checksum, and generated source archives publicly available at that GitHub
release. It does not make the historical Mac binary public and does not
establish Microsoft Store availability.
