...
Status: claim-control document with two preserved measured development benchmarks and one frozen-commit Windows x64 preflight; formal holdout not run
Created: 2026-08-31  
...
| Built a working end-to-end analysis product | TESTED — historical exact-DMG E2E plus frozen-commit Windows x64 preflight | Historical Mac records plus `evidence/releases/0.1.0/windows/windows-preflight-run-33403432371.md` and its packaged E2E report | Historical exact-DMG workflow; Windows Actions run `33403432371` launched `win-unpacked/CausalPilot AI.exe` and exercised mapping, bundled local analysis, results, and export | App 0.1.0; historical Mac artifact has no build-time source commit; Windows preflight commit `f9b0d72716e4958f5b1d523eb926451707437dc6` | “Built and tested an offline workflow on the recorded build Mac and packaged Windows-hosted runner” | “Production-ready”, “enterprise-grade”, or clean-target Windows/macOS compatibility |
...
| Provides a Marketing Analytics case study | TESTED — synthetic demonstration only | Frozen sample metadata, historical exact-DMG result, and Windows packaged E2E result | Re-run the packaged synthetic checkout-incentive workflow and verify dataset SHA-256 `47ab1a639eecfbb1630063298d6d3b447bac5123e87d6e9653bdc283bf9de244` | Synthetic case version/hash, app 0.1.0, platform/artifact identity; Windows preflight has commit `f9b0d727…`, historical Mac build does not | “Demonstrated a synthetic marketing A/B workflow with the recorded question, method, result, and limitations” | Invented client, revenue, adoption, or real-world uplift |
...
| Packaged a desktop or installable release | PACKAGED — historical arm64 DMG and frozen-commit Windows x64 preflight | Historical Mac package record/checksums plus `evidence/releases/0.1.0/windows/windows-preflight-run-33403432371.md` and `SHA256SUMS-windows-preflight.txt` | Historical DMG hash/verify/mounted E2E; Windows run `33403432371` built and hashed NSIS + ZIP from commit `f9b0d727…` | App 0.1.0; Mac arm64 SHA-256 `c5e6d656…`; Windows x64 installer `1af7f1f9…` and ZIP `bb1c9869…` | “Packaged and internally tested CausalPilot AI 0.1.0 for Apple silicon and as a Windows x64 preflight” | “Installed on a clean target”, trusted-signed, “public binary release”, or Store-available |
| Installed and launched on a clean target | TESTED — mounted Mac launch and Windows `win-unpacked` runner launch; not INSTALLED | Historical Mac launch record plus Windows preflight and packaged E2E report | The DMG was launched from a read-only mount on the build Mac; `win-unpacked/CausalPilot AI.exe` was launched on the GitHub-hosted runner; separate clean-device installation remains required | Artifact/commit identity, platform/architecture, test date | “Launched the recorded packaged artifacts and completed the synthetic workflow in their recorded environments” | “Clean-installed”, general Windows 10/11 compatibility, “works on all Macs”, or Intel compatibility |
...
The Windows x64 preflight is traceable to source commit
`f9b0d72716e4958f5b1d523eb926451707437dc6`. Its NSIS installer and ZIP were
built and tested in GitHub Actions run `33403432371`; the preserved record and
screenshots are under `evidence/releases/0.1.0/windows/`.
